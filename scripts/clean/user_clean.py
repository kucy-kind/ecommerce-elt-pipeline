...
# 必须非空的列
NOT_NULL_COLUMNS = ['user_id','user_name' ]
...
def main(target_date):
    ...
    log.info(f"开始清洗 {target_date} 的 users 数据")

    # 1. 从 MinIO 读取当天所有 ODS 数据
    dfs = read_minio(target_date)
    if 'users' not in dfs:
        log.error(f"未找到 users 表的数据，可用表: {list(dfs.keys())}")
        return False

    df_users = dfs['users']
    log.info(f"原始数据形状: {df_users.shape}")

    # 2. 列名检查
    unmarked, all_cols = return_name_array(df_users, log, set(REQUIRED_COLUMNS))
    if unmarked:
        log.warning(f"存在不需要的列: {unmarked}，将在清洗步骤中删除")

    # 3. 清洗数据：删除不需要的列、删除空行
    df_cleaned = clean_df(df_users, REQUIRED_COLUMNS, NOT_NULL_COLUMNS, log)
        
    #4.csv数据内部检查主键唯一，有冲突就进行修改
    df_cleaned=fix_duplicate_primary_key(df_cleaned, 'user_id', log)
